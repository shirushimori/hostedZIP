...
const tabs = ['General', 'Appearance', 'Visual Editor', 'Downloads', 'Advanced']
  ...
  const body = (
    <>
      ...
      <div className="sm-body">
        ...
        {tab === 'Visual Editor' && (
          <div className="sm-section">
            <h3 className="ve-title">Custom Theme Builder</h3>
            
            <SettingRow label="Accent Color">
              <input 
                type="color" 
                value={draft.customAccent || currentTheme.colors.accent} 
            ...
            <SettingRow label="Background Color">
              <input 
                type="color" 
                value={draft.customBg || currentTheme.colors.bg} 
            ...
            <SettingRow label="Container Background">
              <input 
                type="color" 
                value={draft.customBg2 || currentTheme.colors.bg2} 
                onChange={e => set('customBg2', e.target.value)} 
                className="ve-color-picker"
              />
            </SettingRow>
            ...
            <SettingRow label="Text Color">
              <input 
                type="color" 
                value={draft.customText || currentTheme.colors.text} 
                onChange={e => set('customText', e.target.value)} 
                className="ve-color-picker"
              />
            </SettingRow>

            <SettingRow label="Border Corner Radius">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}>
                <input 
                  ...
                  max="24" 
                  step="1" 
                  value={draft.customBorderRadius !== undefined ? draft.customBorderRadius : 8}
                  onChange={e => set('customBorderRadius', Number(e.target.value))} 
                  className="slider" 
                />
                <span style={{ fontSize: 12, minWidth: 32 }}>{(draft.customBorderRadius !== undefined ? draft.customBorderRadius : 8)}px</span>
              </div>
            </SettingRow>

            <SettingRow label="Layout Scale">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}>
                <input 
                  type="range" 
                  min="80" 
                  ...
                  className="slider" 
                />
                <span style={{ fontSize: 12, minWidth: 32 }}>{draft.uiScale}%</span>
              </div>
            </SettingRow>

            <div style={{ marginTop: 10, display: 'flex', justifyContent: 'flex-end' }}>
              <button className="sm-btn danger" style={{ fontSize: 11, padding: '4px 10px' }} onClick={() => {
                setDraft({
                  ...draft,
        ...
        {tab === 'Advanced' && (
          <div className="sm-section">
            <SettingRow label="Developer Mode">
              <Toggle checked={draft.developerMode} onChange={v => set('developerMode', v)} />
            </SettingRow>
