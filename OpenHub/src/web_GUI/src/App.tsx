...
import type { ServiceData, Source, ProgressResponse, SrvResponse } from './types'
import { getService } from './api'
import { loadSettings, saveSettings, applyTheme, type UserSettings } from './theme'
...
export default function App() {
  ...
  const blocked = running || installing

  const btnLabel = !installed ? 'Install' : (versionMatch ? 'Run' : 'Update')
    ...
    fetch('/data/sources.json')
      ...
      .catch(() => {
        getService().then(r => {
          ...
          if (r.download_size) setExactSize(r.download_size)
          fetchServiceInfo()
        })
      })
  }, [])
  ...
  const fetchServiceInfo = useCallback(async () => {
    try {
      ...
      const svc: SrvResponse = await r.json()
      setInstalled(svc.installed)
      setVersionMatch(svc.version_match)
      setLocalVer(svc.local_version)
      if (svc.download_size) setExactSize(svc.download_size)
    } catch {
      // ignore
    }
  }, [])
  ...
  const handleRun = () => {
    ...
    // Minimize the client window when the service launches
  ...
  const doRunCheck = async () => {
    ...
    try {
      ...
      if (!svc.installed || !svc.version_match) {
  ...
  const handleRunTool = async (name: string, _asAdmin = false) => {
    if (blocked) return
    setStatus('Cleaning...')
    try {
      const r = await fetch(`/api/run-tool-terminal/${encodeURIComponent(name)}`, { method: 'POST' })
      const res: { done: boolean; error?: string } = await r.json()
      if (res.error) {
        setStatus('Failed')
        setErrorDialog({ title: 'Tool Failed', message: res.error })
      } else {
        setStatus('Cleaned')
      }
    } catch (e) {
      setStatus('Failed')
      setErrorDialog({ title: 'Tool Failed', message: `Tool failed: ${e instanceof Error ? e.message : String(e)}` })
    }
  }
  ...
  return (
    <div onContextMenu={handleContextMenu}>
            ...
            <>
              <div className="content-top">
                <Header
                  ...
                  version={localVer || data?.ServiceVersion || '1.0.0'}
              ...
              <div className="bottom-bar">
                ...
                <div className="bottom-row">
                  <div className="bottom-info-block">
                    ...
                    {data && (
                      <div className="bottom-meta-details">
                        <span className="bottom-meta-text">Download Size: <strong>{exactSize || '~22 MB'}</strong></span>
                        <span className="bottom-meta-text">Version: <strong>{localVer || data.ServiceVersion || '1.0.0'}</strong></span>
                      </div>
                    )}
                  </div>
                  <RunButton
                    onClick={doRunCheck}
                    loading={!running && loadingSource}
      ...
      {hudEditorOpen && <HudEditor onClose={() => setHudEditorOpen(false)} />}
